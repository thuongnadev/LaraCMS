...
trait HasLocationAction
{
    ...
    public function getLocationAction(): Action
    {
        return Action::make('locations')
            ->label(__('menu::menu-builder.actions.locations.label'))
            ->modalHeading(__('menu::menu-builder.actions.locations.heading'))
            ->modalDescription(__('menu::menu-builder.actions.locations.description'))
            ->modalSubmitActionLabel(__('menu::menu-builder.actions.locations.submit'))
            ...
            ->action(function (array $data) {
                ...
                Notification::make()
                    ->title(__('menu::menu-builder.notifications.locations.title'))
                    ->success()
                    ->send();
            })
            ->form($this->getRegisteredLocations()->map(
                fn ($location, $key) => Components\Grid::make(2)
                    ...
                    ->schema([
                        Components\TextInput::make('location')
                            ->label(__('menu::menu-builder.actions.locations.form.location.label'))
                            ->hiddenLabel($key !== $this->getRegisteredLocations()->keys()->first())
                            ->disabled(),

                        Components\Select::make('menu')
                            ->label(__('menu::menu-builder.actions.locations.form.menu.label'))
                            ->searchable()
                            ->hiddenLabel($key !== $this->getRegisteredLocations()->keys()->first())
                ...
                Components\View::make('filament-tables::components.empty-state.index')
                    ->viewData([
                        'heading' => __('menu::menu-builder.actions.locations.empty.heading'),
